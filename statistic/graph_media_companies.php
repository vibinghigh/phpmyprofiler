...
if ( $pmp_thumbnail_cache && is_file('../cache/graph_media_companies_'.$_SESSION['lang_id'].'.png') ) {
    $filename = '../cache/graph_media_companies_'.$_SESSION['lang_id'].'.png';
    ...
    $query = 'SELECT name AS company, data AS count FROM pmp_statistics WHERE type = \'company_top10\' ORDER BY LPAD(data, 20, \'0\') DESC';
    $result = dbexec($query);
	...
	// Setup the graph
	$graph = new Graph(550, 160, 'graph_media_companies_'.$_SESSION['lang_id'].'.png');
