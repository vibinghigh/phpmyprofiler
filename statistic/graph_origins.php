...
if ( $pmp_thumbnail_cache && is_file('../cache/graph_origins_'.$_SESSION['lang_id'].'.png') ) {
    $filename = '../cache/graph_origins_'.$_SESSION['lang_id'].'.png';
    ...
    $query = 'SELECT name AS origin, data AS count FROM pmp_statistics WHERE type = \'origins\' ORDER BY LPAD(data, 20, \'0\') DESC';
    $result = dbexec($query);
	...
	while ( $row = mysqli_fetch_object($result) ) {
	    $Origin[] = $row->count;
		$Legend[] = entity_to_decimal_value(trunc(t($row->origin), 30));
	}
	...
	// Setup the graph
	$graph = new Graph(550, (12*$rows+40), 'graph_origins_'.$_SESSION['lang_id'].'.png');
