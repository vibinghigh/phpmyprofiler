...
if ( $pmp_thumbnail_cache && is_file('../cache/graph_placeofpurchase_'.$_SESSION['lang_id'].'.png') ) {
    $filename = '../cache/graph_placeofpurchase_'.$_SESSION['lang_id'].'.png';
    ...
    $query = 'SELECT name AS purchplace, data AS count FROM pmp_statistics WHERE type = \'purchplace_top10\' ORDER BY LPAD(data, 20, \'0\') DESC';
    $result = dbexec($query);
	...
	while ( $row = mysqli_fetch_object($result) ) {
	    $Purchplace[] = $row->count;
	    $Legend[] = entity_to_decimal_value(trunc(t($row->purchplace), 18));
	}
	...
	// Setup the graph
	$graph = new Graph(550, (12*$rows+40), 'graph_placeofpurchase_'.$_SESSION['lang_id'].'.png');
