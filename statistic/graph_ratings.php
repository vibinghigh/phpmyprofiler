...
if ( $pmp_thumbnail_cache && is_file('../cache/graph_ratings_'.$_SESSION['lang_id'].'.png') ) {
    $filename = '../cache/graph_ratings_'.$_SESSION['lang_id'].'.png';
    ...
    $query = 'SELECT name AS rating, data AS count FROM pmp_statistics WHERE type = \'ratings\' ORDER BY LPAD(data, 20, \'0\') DESC';
    $result = dbexec($query);
	...
	// Setup the graph
	$graph = new Graph(550, (12*$rows+40), 'graph_ratings_'.$_SESSION['lang_id'].'.png');
